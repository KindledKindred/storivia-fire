...
export default new Vuex.Store({
  state: {
    ...
    congrats: [
      ...
      },
      {
        id: 1,
        icon: "edit",
        message: "庇護者の不在は，束の間の解放と危険をもたらす……"
      },
      {
        id: 2,
        icon: "edit",
      ...
      },
      {
        id: 7,
        icon: "edit",
        message: "主人公や仲間が，意図してかせずか，敵対者を助けてしまった！"
      },
      {
        id: 8,
        icon: "edit",
  ...
  actions: {
    // ADD_XXX の定義
    [types.ADD_ACTION]({ commit, state }, { function31_id, character_id, world_id, abstract, note }) {
      let newAction = {
    ...
    [types.ADD_CHARACTER]({ commit, state }, { name, role_id, age, sex, note }) {
      let newCharacter = {
  ...
  getters: {
    ...
    getFunction31DescriptionById: (state, getters) => id => {
      const function31 = getters.getFunction31byId(id);
      return function31 ? function31.description : "";
    },
    getCongratsMessageById: (state, getters) => id => {
      const congrats = getters.getCongratsById(id);
      return congrats ? congrats.message : "";
    }
  },
