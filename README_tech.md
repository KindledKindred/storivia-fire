...
データ管理ライブラリ．
「データの一元管理」という設計思想を持ちますが，今回は Firestore をメインとして，
完全に静的なデータを Vuex に持たせる運用に留めています．
...
Grid が 12 を超えても改行されないなどの違いもあります．
...
色々サポートしていますが今回は Firestore と Hosting をメインに使用しています．
...
#### なぜ Vuex による一元管理を行わないのか

小規模なシステムのため Firebase のクエリを
各コンポーネントから叩く方が楽だったためです．

もちろん並列処理が走るような複雑なプロジェクトでは
Vuex で全てを管理したほうが堅牢になるでしょう．
